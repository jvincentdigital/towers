  ...
  if (!product) {
    return (
      <main className="max-w-7xl mx-auto px-4 py-20 text-center">
        <h1 className="text-2xl font-bold mb-2 text-neutral-900 dark:text-white">Product not found</h1>
    ...
    <main className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
        ...
        <div className="flex flex-col">
          <p className="text-sm text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-1">
            {product.brand}
          </p>
          <h1 className="text-3xl font-bold tracking-tight mb-3 text-neutral-900 dark:text-white">{product.name}</h1>
          <p className="text-2xl font-semibold text-accent mb-6">
            ${product.price.toFixed(2)}
          </p>
          <p className="text-neutral-600 dark:text-neutral-400 leading-relaxed mb-8">{product.description}</p>

          {/* Size selector */}
          {product.sizes && product.sizes.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold uppercase tracking-wider mb-2 text-neutral-900 dark:text-white">Size</h3>
          ...
          {/* Quantity selector */}
          <div className="mb-8">
            <h3 className="text-sm font-semibold uppercase tracking-wider mb-2 text-neutral-900 dark:text-white">
              Quantity
            </h3>
            <div className="inline-flex items-center border border-neutral-300 dark:border-neutral-600 rounded">
              <button
                onClick={decrement}
                className="px-3 py-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors text-neutral-700 dark:text-neutral-300"
                aria-label="Decrease quantity"
              >
                <Minus className="h-4 w-4" />
              </button>
              <span className="px-4 py-2 min-w-[3rem] text-center text-sm font-medium text-neutral-900 dark:text-white">
                {quantity}
              </span>
              <button
                onClick={increment}
      ...
      {relatedProducts.length > 0 && (
        <section className="mt-20">
          <h2 className="text-2xl font-bold tracking-tight mb-8 text-neutral-900 dark:text-white">Related Products</h2>
